...
// Función para reconstruir detalleVenta desde múltiples partes en la URL
function reconstructDetalleVenta(urlParams) {
    let detalleVenta = '';
    let index = 1;
    while (urlParams.get(`detalleVenta${index}`)) {
        detalleVenta += urlParams.get(`detalleVenta${index}`);
        index++;
    }
    return detalleVenta;
}
...
function getUrlParameters() {
    ...
    const params = {
        detalleVenta: reconstructDetalleVenta(urlParams), // Reconstruir detalleVenta
